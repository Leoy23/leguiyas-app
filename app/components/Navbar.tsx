  ...
  return (
    <nav className={`w-full md:p-6 md:flex md:justify-center`}>
      <header className={`flex justify-end items-center text-xs md:text-base`}>
        ...
        <ul
          className={`text-center md:flex md:justify-center md:items-center md:gap-24 lg:gap-32 fixed md:static transition-all duration-500 ease-in-out ${
            open
              ? "bg-darkPink top-16 opacity-100 gap-2 p-6"
              : "-top-[490px] md:opacity-100 opacity-0"
          }`}
          onScroll={handleRouteChange}
        >
          {navLinks.map((link, index) => (
            <li
              key={index}
              className={`pb-2 ${
                link.toUpperCase() === "CONTACT" &&
                "rounded-full bg-pink py-2 px-2"
              }`}
              onClick={(e) => {
                e.preventDefault();
                setOpen(false);
